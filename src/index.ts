...
function getRouteDescription(method: string, path: string): string {
  const descriptions: { [key: string]: string } = {
    ...
    'POST /prompt': 'Process Claude AI prompts with optional repository context',
    'GET /claude/sessions': 'List all Claude conversation sessions',
    'GET /sessions': 'List all Claude conversation sessions',
    'POST /claude/sessions': 'Create new Claude conversation session with optional repository context',
    'POST /sessions': 'Create new Claude conversation session with optional repository context',
    'GET /claude/sessions/:id': 'Get specific Claude session conversation history',
    ...
    'POST /claude/sessions/:id/prompt': 'Send prompt to existing Claude session',
...
app.get("/api", (req, res) => {
  ...
  Object.entries(routes).forEach(([route, description]) => {
    if (route.includes('/git/') || (route.includes('/repos') || route.includes('/init') || route.includes('/save') || route.includes('/pull') || route.includes('/branch') || route.includes('/checkout') || route.includes('/log') || route.includes('/reset') || route.includes('/diff') || route.includes('/merge') || route.includes('/stash') || route.includes('/tag') || route.includes('/remote')) && !route.includes('/claude/')) {
      groupedRoutes.git[route] = description;
    } else if (route.includes('/claude/') || (route.includes('/prompt') || route.includes('/sessions')) && !route.includes('/git/')) {
